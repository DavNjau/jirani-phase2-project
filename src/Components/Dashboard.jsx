...
function Dashboard({ businesses, allBusinesses, filteredBusinesses, setAllBusinesses, setFilteredBusinesses }) {
  ...
  }, [setAllBusinesses, setFilteredBusinesses]);

  const handleDelete = (id) => {
    fetch(`http://localhost:3001/businesses/${id}`, { method: "DELETE" })
      .then(() => {
        const updatedAll = allBusinesses.filter((b) => b.id !== id);
        const updatedFiltered = filteredBusinesses.filter((b) => b.id !== id);
